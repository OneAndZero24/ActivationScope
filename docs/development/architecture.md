...
2. **Native libtorch hooks** — Hooks are registered via Python's ``register_forward_hook`` API, but the callback is a pybind11 thunk that extracts the tensor under GIL, then releases the GIL and calls a C++ hot path. No Python callables are invoked during reduction.
...
- `csrc/reduction.hpp/.cpp` — TorchScript module wrapper loaded from `.pt` file.
